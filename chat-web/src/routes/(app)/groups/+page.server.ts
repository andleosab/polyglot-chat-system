...
async function fetchUserGroups(uuid: string | undefined, fetch: typeof globalThis.fetch) {
    const url = new URL(`${API.messages}/groups?id=${uuid}`);
...
export const load: PageServerLoad = async ({ locals, fetch }) => {
    ...
    console.log('Loading groups page with base url:', API.messages);

	try {

		let data = await fetchUserGroups(uuid, fetch);
		...
		if (data.length === 0) {
			console.warn('No groups found for the given user.');
		}
